...
public class ArrayUtil {
	...
	/**
	 * The main loop in the method is figuring out all the tiles from the start to
	 * the end. When we have the last direction with the current tile and the
	 * previous tile, we can place that into the roadDirArray [local variable]. This
	 * way we have the array (y, x) coordinate storing the direction to the next
	 * road.
	 *
	 * In the and we set the array's end point with the last direction to guarantee
	 * that the mob will move until the end of the map and reduce the player's
	 * lives.
	 *
	...
	/**
	 * Testing diferent directions until find the next tile in the sequence and
	 * return that tile.
	 *
	 * @param prevTile
	 * @param lastDir
	...
	private static boolean isTileRoad(PathPoint testTile, int[][] lvlTypeArr) {
		return testTile != null && testTile.getYCord() >= 0 && testTile.getYCord() < lvlTypeArr.length
